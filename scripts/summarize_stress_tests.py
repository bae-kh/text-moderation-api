...
def format_float(value: str, digits: int = 2) -> str:
...
def main() -> None:
    ...
    for users in SCENARIOS:
        csv_path = RESULT_DIR / f"stress_users_{users}_stats.csv"
        ...
        summary_rows.append({
            "Users": str(users),
            "Avg Latency (ms)": format_float(row.get("Average Response Time", "0")),
            "p95 Latency (ms)": format_float(row.get("95%", "0")),
            "p99 Latency (ms)": format_float(row.get("99%", "0")),
            "RPS": format_float(row.get("Requests/s", "0")),
            "Failure Rate (%)": f"{failure_rate:.2f}",
        })

    output_path = RESULT_DIR / "stress_summary.md"
    ...
    print(f"Stress summary written to {output_path}")
